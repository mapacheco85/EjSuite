...
Namespace EjSuite.Modules.EjSuite
    Public Class EjSuitePagosCuenta
        ...
        Protected Sub cmdInsertPago_Click(ByVal sender As Object, ByVal e As EventArgs) Handles cmdInsertPago.Click
            Try
                ...
                Dim fechaaux As Date = CDate(txtFechaPago.Text)
                If Page.IsValid AndAlso PagoCliente1.ClienteId.ToString <> "" AndAlso PagoCliente1.nCodFactura.ToString <> "" AndAlso (CDec(txtMontoPago.Text) > 0 AndAlso CDec(txtMontoPago.Text) <= CDec(PagoCliente1.SaldoTextValue)) AndAlso facto.DFechaEmision <= fechaaux Then
                    Dim objNewPago As New PagoCuentaXCobrar()
                    objNewPago.NFacturaId = PagoCliente1.nCodFactura
                    objNewPago.NMontoPagado = CDec(txtMontoPago.Text)
                    'objNewPago.Comprobante=
                    objNewPago.CBancoCobro = ""
                    objNewPago.BCheque = False
        ...
        Protected Sub grdBusquedaPagos_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles grdBusquedaPagos.SelectedIndexChanged
            If grdBusquedaPagos.SelectedIndex > -1 Then
                ...
                If objPago.DFechaPago = Now.ToShortDateString And objPago.BCheque = False Then
                ...
                Else
                    EjSuite.Mensaje(Me.upPanel, Me.GetType(), "¡Solo se pueden modificar los pagos al contado efectuados el dia de hoy!")
                End If
            End If
        End Sub
        ...
        Protected Sub cmdUpdatePago_Click(ByVal sender As Object, ByVal e As EventArgs) Handles cmdUpdatePago.Click
            If UserController.Instance.GetCurrentUserInfo().Username = "ejimenez" Then
                Dim facto As New Factura(PagoCliente1.nCodFactura)
                Dim fechaaux As Date = CDate(txtFechaPago1.Text)

                If ViewState("pagoid") IsNot Nothing AndAlso Page.IsValid AndAlso VendedorControlSearch2.EmpleadoId.ToString <> "" AndAlso facto.DFechaEmision <= fechaaux AndAlso (CDec(txtMontoPago1.Text) > 0 AndAlso CDec(txtMontoPago1.Text) <= CDec(PagoCliente2.SaldoTextValue)) Then
        ...
        Protected Sub lbtnXLS_Click(ByVal sender As Object, ByVal e As EventArgs) Handles lbtnXLS.Click
            If grdBusquedaPagos.Rows.Count > 0 Then
                GridViewExportUtil.Export("ReportePagosContado.xlsx", grdBusquedaPagos)
            End If
        End Sub
        ...
        Protected Sub lbtn2_Click(sender As Object, e As EventArgs) Handles lbtn2.Click
            ...
            If AdminSettings Or EjSuite.ControlarAccesoUsuarios("ejs_managers") Or EjSuite.ControlarAccesoUsuarios("ejs_auxiliar") Then
                pnlPagos.Visible = False
